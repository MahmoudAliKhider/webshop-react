...
export const getAllOrders = (page, limit) => async (dispatch) => {
    try {
        const response = await useGetDataToken(`/orders?limit=${limit}&page=${page}`);
        dispatch({
            type: GET_ALL_ORDER,
            payload: response,
        })
...
export const changeOrderPay = (id) => async (dispatch) => {
    try {
        const response = await useInsUpdateData(`/orders/${id}/pay`);
...
export const changeOrderDeliver = (id) => async (dispatch) => {
    try {
        const response = await useInsUpdateData(`/orders/${id}/deliver`);
